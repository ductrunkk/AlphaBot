...
// L298N H-Bridge Connection PINs (ESP32 DevKit V1)
#define L298N_enA  26  // PWM (GPIO26)
#define L298N_enB  27  // PWM (GPIO27)
#define L298N_in4  13  // Dir Motor B
#define L298N_in3  12  // Dir Motor B
#define L298N_in2  14  // Dir Motor A
#define L298N_in1  15  // Dir Motor A

// Wheel Encoders Connection PINs
#define right_encoder_phaseA  2   // Interrupt-capable pin
#define right_encoder_phaseB  4   // GPIO
#define left_encoder_phaseA   5   // Interrupt-capable pin
#define left_encoder_phaseB   18  // GPIO
...
const unsigned long interval = 100; // 100ms sampling interval
...
// PID
// Setpoint - Desired
double right_wheel_cmd_vel = 0.0;     // rad/s (max ~26.18 rad/s under load)
double left_wheel_cmd_vel = 0.0;      // rad/s (max ~26.18 rad/s under load)
// Input - Measurement
...
// Tuning (may need adjustment for your motor)
double Kp_r = 11.5;
double Ki_r = 7.5;
double Kd_r = 0.1;
double Kp_l = 12.8;
...
// Khai báo trước cho các hàm ngắt
...
void setup() {
  ...
  // Set up PWM channels for ESP32 (version 3.x)
  ledcAttach(L298N_enA, PWM_FREQ, PWM_RESOLUTION); // Attach PWM to pin enA
  ledcAttach(L298N_enB, PWM_FREQ, PWM_RESOLUTION); // Attach PWM to pin enB
...
void loop() {
  ...
  if (current_millis - last_millis >= interval) {
    // Velocity calculation: (pulses / interval) * (60 / pulses_per_rev) * (2π / 60)
    // Interval = 100ms (0.1s), so pulses per second = 10 * counter
    // Pulses per revolution = 330#include <PID_v1.h>
...
void leftEncoderCallback()
{
  ...
  left_encoder_counter++;
    ...
    // Write PWM to motors
    ledcWrite(L298N_enA, (int)right_wheel_cmd);
    ledcWrite(L298N_enB, (int)left_wheel_cmd);
  }
}
